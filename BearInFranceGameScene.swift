//
//  BearInFranceGameScene.swift
//  Benjamin and Ninjamen
//
//  Created by Jonathan Andrew Hendrix on 2/12/17.
...
class BearInFranceGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "bearInFrance")
    var hitCounter = 0
    ...
    override func didMove(to view: SKView) {
        ...
        //Player
        player.position = CGPoint(x: size.width * 0.505, y: size.height * 0.385)//Benjamin CGPoint(x: size.width/2, y: size.height * 0.413)
        player.zPosition = 3
        addChild(player)
        
        //Player Damage
        playerDamage.position = CGPoint(x: size.width/2, y: size.height * 0.555)
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"QueenBear")}
    ...
    func nextRound(){
        ...
        let nextScene = BearInTokyoGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
