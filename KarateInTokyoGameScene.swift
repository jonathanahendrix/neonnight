...
class KarateInTokyoGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "karateInTokyo")
    var hitCounter = 0
    ...
    override func didMove(to view: SKView) {
        ...
        //Enemy
        enemy.position = CGPoint(x:size.width/2, y:size.height * 0.73)//Crogan
        enemy.zPosition = 3
        addChild(enemy)
        ...
        //Enemy Damage
        enemyDamage.position = CGPoint(x: size.width/2.02, y: size.height * 0.565)
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"CroganKarate")}
    ...
    func nextRound(){
        gameStatus = false
        let nextScene = KarateEndingStoryScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        let changeScene = SKAction.run({self.scene?.view?.presentScene(nextScene, transition: transition)})
