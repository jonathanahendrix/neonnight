...
class KobeInFranceGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "kobeInFrance")
    var hitCounter = 0
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"QueenKobe")}
    ...
    func nextRound(){
        ...
        let nextScene = KobeInTokyoGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
