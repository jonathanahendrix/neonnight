...
class BearInNewYorkGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "bearInNewYork")
    var hitCounter = 0
    ...
    override func didMove(to view: SKView) {
        ...
        //Enemy
        enemy.position = CGPoint(x:size.width/2, y:size.height * 0.728)//Croggan CGPoint(x:size.width/2, y:size.height * 0.73)//Echo CGPoint(x:size.width/2, y:size.height * 0.739)//Queen CGPoint(x:size.width/2, y:size.height * 0.72)//Hobo CGPoint(x:size.width/2, y:size.height * 0.728)
        enemy.zPosition = 3
        addChild(enemy)
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"HoboBear")}
    ...
    func nextRound(){
        ...
        let nextScene = BearInBrazilGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
