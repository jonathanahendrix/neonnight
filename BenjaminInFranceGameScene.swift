//
//  BenjaminInFranceGameScene.swift
...
class BenjaminInFranceGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "benjaminInFrance")
    var hitCounter = 0
    ...
    override func didMove(to view: SKView) {
        ...
        //Player
        player.position = CGPoint(x: size.width/2, y: size.height * 0.413)//Kobe CGPoint(x: size.width/2, y: size.height * 0.40)//Karate CGPoint(x: size.width/2, y: size.height * 0.413) //Bear CGPoint(x: size.width * 0.505, y: size.height * 0.385)//Benjamin CGPoint(x: size.width/2, y: size.height * 0.413)
        player.zPosition = 3
        addChild(player)
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"QueenBen")}
