...
//
//  Created by Jonathan Andrew Hendrix on 1/29/18.
...
class Queen: SKSpriteNode {
    ...
    var textureImage:SKTexture =
        SKTexture(imageNamed:"QueenIdle" )
    var tell1 = SKAction()
    var tell2 = SKAction()
    var tell3 = SKAction()
    var punchRight = SKAction()
    var punchLeft = SKAction()
    var rightPunch = SKSpriteNode(imageNamed:"QueenRightArm")
    var leftPunch = SKSpriteNode(imageNamed:"QueenLeftArm")
    var idle = SKAction()
    var enemyDamage = SKSpriteNode(imageNamed:"enemyDamage")
    var maxHitAmount:Int = 2500
    init(){
        ...
        leftPunch.position = CGPoint(x:-20, y:-11)
        ...
        rightPunch.position = CGPoint(x:20, y:-10)
    ...
    func createIdle(){
        let addIdle = SKAction.run {self.texture = SKTexture(imageNamed:"QueenIdle")}
        let wait2 = SKAction.wait(forDuration: 1.40)
        idle = SKAction.sequence([addIdle,wait2])
        
    }
    func createTell1(){
        let addTell = SKAction.run {self.texture = SKTexture(imageNamed:"QueenTell1") }
        let wait3 = SKAction.wait(forDuration: 0.50)
        tell1 = SKAction.sequence([addTell,wait3])
    }
    
    func createTell2(){
        let addTell = SKAction.run {self.texture = SKTexture(imageNamed:"QueenTell2") }
        let wait3 = SKAction.wait(forDuration: 0.50)
    ...
    func createTell3(){
        let addTell = SKAction.run {self.texture = SKTexture(imageNamed:"QueenTell3") }
        let wait3 = SKAction.wait(forDuration: 0.50)
        tell3 = SKAction.sequence([addTell,wait3])
    }
    
    func createLeftPunch(){
        let addPunch = SKAction.run {self.texture = SKTexture(imageNamed:"QueenLeftPunch") }
    ...
    func createRightPunch(){
        let addPunch = SKAction.run {self.texture = SKTexture(imageNamed:"QueenRightPunch") }
