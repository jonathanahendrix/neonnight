//
//  BenjaminInBrazilGameScene.swift
//  Benjamin and Ninjamen
//
//  Created by Jonathan Andrew Hendrix on 3/27/17.
//  Copyright © 2017 Neon Night Riders. All rights reserved.
//

import SpriteKit

class BenjaminInBrazilGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "benjaminInBrazil")
    var hitCounter = 0
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"EchoBen")}
    ...
    func nextRound(){
        ...
        let nextScene = BenjaminInFranceGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
