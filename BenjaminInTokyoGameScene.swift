//
//  BenjaminInTokyoGameScene.swift
...
class BenjaminInTokyoGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "benjaminInTokyo")
    var hitCounter = 0
    ...
    func nextRound(){
        ...
        let nextScene = BenjaminEndingStoryScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        let changeScene = SKAction.run({self.scene?.view?.presentScene(nextScene, transition: transition)})
