//
//  BearBrazilGameScene.swift
//  Benjamin and Ninjamen
//
//  Created by Jonathan Andrew Hendrix on 1/23/17.
//  Copyright © 2017 Neon Night Riders. All rights reserved.
//
import SpriteKit


class BearInBrazilGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "bearInBrazil")
    var hitCounter = 0
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"EchoBear")}
    ...
    func nextRound(){
        ...
        let nextScene = BearInFranceGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
