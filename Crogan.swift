...
class Crogan: SKSpriteNode {
    
    var textureImage:SKTexture =
        SKTexture(imageNamed:"CroganIdle" )
    var tell1 = SKAction()
    var tell2 = SKAction()
    var tell3 = SKAction()
    var punchRight = SKAction()
    var punchLeft = SKAction()
    var rightPunch = SKSpriteNode(imageNamed:"CroganRightArm")
    var leftPunch = SKSpriteNode(imageNamed:"CroganLeftArm")
    var idle = SKAction()
    var enemyDamage = SKSpriteNode(imageNamed:"enemyDamage")
    var maxHitAmount:Int = 3000
    ...
    init(){
        ...
        leftPunch.position = CGPoint(x:-23, y:-12)
        ...
        rightPunch.position = CGPoint(x:18, y:-12)
    ...
    func createIdle(){
        let addIdle = SKAction.run {self.texture = SKTexture(imageNamed:"CroganIdle")}
        let wait2 = SKAction.wait(forDuration: 1.40)
        idle = SKAction.sequence([addIdle,wait2])
        
    }
    func createTell1(){
        let addTell = SKAction.run {self.texture = SKTexture(imageNamed:"CroganTell1") }
        let wait3 = SKAction.wait(forDuration: 0.50)
        tell1 = SKAction.sequence([addTell,wait3])
    }
    
    func createTell2(){
        let addTell = SKAction.run {self.texture = SKTexture(imageNamed:"CroganTell2") }
        let wait3 = SKAction.wait(forDuration: 0.50)
    ...
    func createTell3(){
        let addTell = SKAction.run {self.texture = SKTexture(imageNamed:"CroganTell3") }
        let wait3 = SKAction.wait(forDuration: 0.50)
        tell3 = SKAction.sequence([addTell,wait3])
    }
    
    func createLeftPunch(){
        let addPunch = SKAction.run {self.texture = SKTexture(imageNamed:"CroganLeftPunch") }
        let addLeftArm = SKAction.run{self.addChild(self.leftPunch)}
        let wait3 = SKAction.wait(forDuration: 0.4)
    ...
    func createRightPunch(){
        let addPunch = SKAction.run {self.texture = SKTexture(imageNamed:"CroganRightPunch") }
