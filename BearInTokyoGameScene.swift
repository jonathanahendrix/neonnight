//
//  BearInTokyoGameScene.swift
//  Benjamin and Ninjamen
//
//  Created by Jonathan Andrew Hendrix on 4/1/17.
//  Copyright © 2017 Neon Night Riders. All rights reserved.
//
import SpriteKit


class BearInTokyoGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "bearInTokyo")
    var hitCounter = 0
    var gameStatus:Bool = true
    let pauseBackground = SKSpriteNode(imageNamed: "TokyoPause")
    ...
    //Music
    var gameBackgroundMusic: SKAudioNode = SKAudioNode(fileNamed:"Crogan.m4a")
    ...
    override func didMove(to view: SKView) {
        ...
        //Enemy
        enemy.position = CGPoint(x:size.width/2, y:size.height * 0.73)//Echo CGPoint(x:size.width/2, y:size.height * 0.739)//Queen CGPoint(x:size.width/2, y:size.height * 0.72)//Hobo CGPoint(x:size.width/2, y:size.height * 0.728)
        enemy.zPosition = 3
        addChild(enemy)
        ...
        //Enemy Damage
        enemyDamage.position = CGPoint(x: size.width/2.02, y: size.height * 0.566)
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"CroganBen")}
    ...
    func nextRound(){
        ...
        let nextScene = BearEndingStoryScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        let changeScene = SKAction.run({self.scene?.view?.presentScene(nextScene, transition: transition)})
        ...
        self.run(SKAction.sequence([pauseGroup,wait1,backgroundfadeOut,wait2,changeScene ]))
        ...
        /*
         ...
         let nextScene = KobeEndingStoryScene(size: scene!.size)
         let transition = SKTransition.fade(withDuration: 0.5)
         gameStatus = false
