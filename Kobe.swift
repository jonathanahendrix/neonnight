...
//
//  Created by Jonathan Andrew Hendrix on 2/18/18.
...
class Kobe: SKSpriteNode {
    
    var playerImage:SKTexture =
        SKTexture(imageNamed:"KobeIdle" )
    var playerName: String = "Kobe"
    var playerLeftArm = SKSpriteNode(imageNamed:"KobeLeftArm")
    var playerRightArm = SKSpriteNode(imageNamed:"KobeRightArm")
    ...
    init() {
        ...
        //Left Arm Punch
        playerLeftArm.position = CGPoint(x: -38, y: 42)
        ...
        //Right Arm Punch
        playerRightArm.position = CGPoint(x: 37, y: 42)
        ...
        //Player Damage
        //playerDamage.position = CGPoint(x: 0, y: 56)
    ...
    func playerActions(button: SKSpriteNode, screenSize: CGSize){
        ...
        let waitPunch = SKAction.wait( forDuration: 0.2)
        let waitDodge = SKAction.wait(forDuration: 0.5)
        let addIdle = SKAction.run {
            self.texture = SKTexture(imageNamed:"KobeIdle")}
        if button.name == "leftPunch"{
            let addLeftPunch = SKAction.run{self.texture = SKTexture(imageNamed:"KobeLeftPunch")}
            ...
            let addRightPunch = SKAction.run{self.texture = SKTexture(imageNamed:"KobeRightPunch")}
            ...
            let addLeftDodge = SKAction.run{self.texture = SKTexture(imageNamed:"KobeLeftDodge")}
            let moveLeft = SKAction.move(to: CGPoint(x:screenSize.width * 0.343, y:screenSize.height * 0.399), duration: 0.00)
            let moveBack = SKAction.move(to: CGPoint(x: screenSize.width/2, y: screenSize.height * 0.40), duration: 0.00)
            ...
            let addRightDodge = SKAction.run{self.texture = SKTexture(imageNamed:"KobeRightDodge")}
            let moveRight = SKAction.move(to: CGPoint(x:screenSize.width * 0.657, y:screenSize.height * 0.399), duration: 0.00)
            let moveBack = SKAction.move(to: CGPoint(x: screenSize.width/2, y: screenSize.height * 0.40), duration: 0.00)
