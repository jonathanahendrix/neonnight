//
//  KarateInFranceGameScene.swift
//  Benjamin and Ninjamen
//
//  Created by Jonathan Andrew Hendrix on 2/11/17.
//  Copyright © 2017 Neon Night Riders. All rights reserved.
//
import SpriteKit


class KarateInFranceGameScene: SKScene, SKPhysicsContactDelegate {
    let userDefaults = UserDefaults.standard
    let hud = HUD()
    let player = Karate()
    let enemy = Queen()
    var playerDamage = SKSpriteNode(imageNamed:"playerDamage2")
    var enemyDamage = SKSpriteNode(imageNamed:"enemyDamage")
    let background = SKSpriteNode(imageNamed: "karateInFrance")
    var hitCounter = 0
    var gameStatus:Bool = true
    let pauseBackground = SKSpriteNode(imageNamed: "FrancePause")
    ...
    let splashVideo = SKVideoNode(fileNamed: "QueenDefeat.mp4")
    var maxHitCount = 0
    var currentScore = 0
    
    //Music
    var gameBackgroundMusic: SKAudioNode = SKAudioNode(fileNamed:"Queen.m4a")
    ...
    override func didMove(to view: SKView) {
        ...
        //Enemy
        enemy.position = CGPoint(x:size.width/2, y:size.height * 0.72)//Hobo CGPoint(x:size.width/2, y:size.height * 0.728)
        enemy.zPosition = 3
        addChild(enemy)
        
        //Player
        player.position = CGPoint(x: size.width/2, y: size.height * 0.413) //Bear CGPoint(x: size.width * 0.505, y: size.height * 0.385)//Benjamin CGPoint(x: size.width/2, y: size.height * 0.413)
        player.zPosition = 3
        addChild(player)
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"QueenKarate")}
    ...
    func nextRound(){
        ...
        let wait2 = SKAction.wait(forDuration: 7.5)
        let nextScene = KarateInTokyoGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
        ...
        let removeHitIcon = SKAction.run {self.hud.hitIcon.removeFromParent()}
        let removeScore = SKAction.run {self.hud.scoreLabel.removeFromParent()}
        let removePlayer = SKAction.run{self.player.removeFromParent()}
        let removeEnemey = SKAction.run{self.enemy.removeFromParent()}
        let removeGroup = SKAction.group([removeEnemey,removePlayer,removeScore,removeHitIcon])
