//
//  BearCharacterScene.swift
//  Benjamin and Ninjamen
//
//  Created by Jonathan Andrew Hendrix on 1/11/17.
//  Copyright © 2017 Neon Night Riders. All rights reserved.
//
import SpriteKit

class BearCharacterScene: SKScene {
    let background = SKSpriteNode(imageNamed: "bearSelectConfirm")
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let transition = SKTransition.doorsOpenHorizontal(withDuration: 0.03)
        ...
        if benjamin.contains(touchLocation) {
            if userDefaults.bool(forKey: "sound"){
                benjamin.run(SKAction.playSoundFileNamed("Bloop.mp3", waitForCompletion: true))
            }
            ...
            if (userDefaults.bool(forKey: "story")){
                ...
                let nextScene = BearInNewYorkGameScene(size: scene!.size)
                nextScene.scaleMode = .aspectFill
                scene?.view?.presentScene(nextScene, transition: transition)
            } else {
