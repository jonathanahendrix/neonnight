...
class KarateInBrazilGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "karateInBrazil")
    var hitCounter = 0
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"EchoKarate")}
    ...
    func nextRound(){
        ...
        let nextScene = KarateInFranceGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
