...
class KobeInNewYorkGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let background = SKSpriteNode(imageNamed: "kobeInNewYork")
    var hitCounter = 0
    ...
    func gameOver() {
        ...
        gameStatus = false
        let gameOverScene = SKAction.run{self.background.texture = SKTexture(imageNamed:"HoboKobe")}
    ...
    func nextRound(){
        ...
        let nextScene = KobeInBrazilGameScene(size: scene!.size)
        let transition = SKTransition.fade(withDuration: 0.5)
        gameStatus = false
